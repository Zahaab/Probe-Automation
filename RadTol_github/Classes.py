...
class ProberController:
    ...
    def move_to_site(self, site_name):
        ...
        if site_name == "AWAY":
            self.prober.move_scope_xy(XyReference.Zero, start_x, site_coords[1])
        elif site_name == "RAD":
            self.prober.move_scope_xy(XyReference.Zero, site_coords[1], start_y)
    ...
    def set_camera_zoom(self, zoom_index):
        """Sets the microscope zoom level."""
        if SIMULATION_MODE:
    ...
    def auto_focus(self):
        """Triggers the Vision Auto Focus."""
        if SIMULATION_MODE:
    ...
    def snap_image_remote(self, remote_path):
        """
        Tells the Prober to save an image to 'remote_path'.
        remote_path must be a valid path on the Prober's PC.
        NOTE: Here we use send_cmd(cmd) to execute the snap 
        command directly, as the API does not have a dedicated 
        method for this. Might need tweaking based on actual capabilities.
        """
        ...
        # Command: vis:snap_image <filename> <include_overlays>
        # Might have to change save dir to something local
        self.prober.send_cmd(f'vis:snap_image "{clean_path}"')
